...
import { ArrowLeft, CheckCircle2, Clock, AlertCircle, ChevronRight, Star } from "lucide-react";
        ...
        <div className="container px-4 py-6 tablet:py-10 max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-3 tablet:gap-4 mb-6">
            <div className="rounded-xl border border-border bg-background p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground">구매 확정</p>
              <p className="text-2xl font-black mt-1">{confirmedCount}</p>
            </div>
            <div className="rounded-xl border border-border bg-background p-4 text-center">
              <Star className="h-5 w-5 mx-auto mb-2 text-star" />
              <p className="text-xs text-muted-foreground">작성 가능한 리뷰</p>
              <p className="text-2xl font-black mt-1">{reviewableCount}</p>
            </div>
            <div className="rounded-xl border border-border bg-background p-4 text-center">
              <AlertCircle className="h-5 w-5 mx-auto mb-2 text-destructive" />
              <p className="text-xs text-muted-foreground">주문 취소</p>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((purchase) => {
                ...
                return (
                  <div
                    key={purchase.id}
                    className="rounded-xl border border-border bg-background p-4 tablet:p-5"
                  >
                    {/* Status & Date */}
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-xs text-muted-foreground">주문일시 {purchase.date}</span>
                    </div>
